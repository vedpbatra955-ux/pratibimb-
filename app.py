...
def generate_production_launch_book(pdf_file):
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function() {{
                ...
                setTimeout(() => {{
                    ...
                    // Detect if device is a mobile phone (switch to 1-page swipe)
                    ...
                    const pageFlip = new St.PageFlip(document.getElementById('book'), {{
                        ...
                        height: bookH,
                        size: "fixed",
                        usePortrait: isMobile,           // Mobile gets single page, PC gets double
                        drawShadow: true,                
                        showCover: true,                 
                        flippingTime: 1100,              
                        maxShadowOpacity: 0.7,
                        mobileScrollSupport: true        // Enables touch-swiping
                    }});
...
if not st.session_state.reader_active:
    ...
    st.markdown("<p style='text-align: center; font-size: 1.2rem; color: #888;'>Upload the official document to prepare the launch environment.</p><br>", unsafe_allow_html=True)
    ...
    with col2:
        ...
        if uploaded_pdf:
            st.write("---")
            if st.button("🚀 PREPARE LAUNCH STAGE", type="primary", use_container_width=True):
                try:
                    st.session_state.html_content = generate_production_launch_book(uploaded_pdf)
                    st.session_state.reader_active = True
                    st.rerun()
                except Exception as e:
                    st.error(f"Failed to prepare launch: {e}")
