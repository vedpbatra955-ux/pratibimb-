...
def generate_production_launch_book(pdf_path, audio_src, logo_src):
    ...
    images_html = ""
    progress_bar = st.progress(0, text=f"{total_pages} पृष्ठों को हाई-डेफिनिशन (HD) में लोड किया जा रहा है...")
...
if not st.session_state.reader_active:
    ...
    with col3:
        if st.button("⚙️ GENERATE OFFLINE E-BOOK", use_container_width=True):
            with st.spinner("ऑफ़लाइन फ़ाइल बनाई जा रही है (इसमें 15-20 सेकंड लग सकते हैं)..."):
                try:
                    audio_source = fetch_sangeet_audio()
                    logo_source = fetch_logo_image()
